...
import { UserDeviceOrientationCamera } from "./UserDeviceOrientationCamera";
...
export class FreeCameraDeviceOrientationFilteredInput
  implements ICameraInput<UserDeviceOrientationCamera> {
  ...
  constructor(threshold?: number) {
    ...
    if (
      ...
      this._device.platform.toLowerCase() != "browser"
    ) {
  ...
  attachControl(element: HTMLElement, noPreventDefault?: boolean) {
    ...
    if (
      ...
      var cam = this;
      this._watchGyro = this._gyro.watch(
        function(gyro: any) {
  ...
  _sensorsRaw(evt: any) {
    ...
    this._calculate(
      -BABYLON.Tools.ToDegrees(values[0]),
      -BABYLON.Tools.ToDegrees(values[1]),
      BABYLON.Tools.ToDegrees(values[2])
    );
  }
  ...
  private _calculate = (alpha: any, beta: any, gamma: any) => {
    ...
    if (
      Math.abs(this._lastDrift - this._drift) > 160 &&
  ...
  private _orientationChanged = () => {
    this._screenOrientationAngle =
      window.orientation !== undefined
        ? +window.orientation
        : window.screen.orientation && (<any>window.screen.orientation)["angle"]
...
(<any>CameraInputTypes)[
  "FreeCameraDeviceOrientationFilteredInput"
] = FreeCameraDeviceOrientationFilteredInput;
